...
function updateCustomScoreDisplay() {
    if (customScoreEl) {
...
function resetTrajectory() {
    ...
    trailMaskPathEl.style.strokeDasharray = '0 1';
...
function animateTrajectory() {
    ...
    const totalLength = Math.max(0.01, trajectoryGuideEl.getTotalLength());
    const firstPoint = trajectoryGuideEl.getPointAtLength(0);

    // Initialize the reveal mask before the visible path is enabled. This avoids
    // the one-frame full-trajectory flash that occurred after selecting a port.
    trailMaskPathEl.style.strokeDasharray = `0 ${totalLength}`;
    pinballDotEl.setAttribute('cx', firstPoint.x);
    pinballDotEl.setAttribute('cy', firstPoint.y);
    ...
    requestAnimationFrame((startTime) => {
        function frame(now) {
            ...
            const currentLength = Math.max(0.001, totalLength * eased);
            const point = trajectoryGuideEl.getPointAtLength(currentLength);

            trailMaskPathEl.style.strokeDasharray = `${currentLength} ${totalLength}`;
...
function buildShareText(completedAllLevels = false) {
    const modeLabel = isHardMode
        ? 'Pinball Recall (Hard Mode)'
        : (isCustomMode ? 'Pinball Recall (Custom)' : 'Pinball Recall');
